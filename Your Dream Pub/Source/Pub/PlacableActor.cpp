...
void APlacableActor::AddPlacedItem()
{
	...
	if (Player)
	{
		...
		{
			FAttributes SavedPlacable;
			SavedPlacable.Placable = this->GetClass();
			SavedPlacable.PlacableTransform = this->GetTransform();
			SaveGameInstance->PlacableStructArray.Add(SavedPlacable);
...
void APlacableActor::RemovePlacedItem()
{
	...
	if (SaveGameInstance != nullptr)
	{
		...
		{
			if (SaveGameInstance->PlacableStructArray[i].Placable == SavedPlacable.Placable && SaveGameInstance->PlacableStructArray[i].PlacableTransform.GetLocation() == SavedPlacable.PlacableTransform.GetLocation())
			{
				SaveGameInstance->PlacableStructArray.RemoveAt(i);
			}
		}
